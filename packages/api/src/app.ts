...
import { assertEvmAddress } from "@kite-security-command-center/core";
...
app.get("/health", (c) => c.json({ ok: true, service: "kite-security-command-center" }));
app.get("/modules", (c) => c.json({ modules }));
app.get("/scan", (c) => c.json({ scans: items, items }));
...
app.get("/safesign", (c) => c.json({ route: "/safesign", product: "KiteSecurity Command Center", preview: true, modules }));
app.get("/incidents", (c) => c.json({ route: "/incidents", product: "KiteSecurity Command Center", preview: true, modules }));
app.get("/revoke", (c) => c.json({ route: "/revoke", product: "KiteSecurity Command Center", preview: true, modules }));
app.get("/settings", (c) => c.json({ route: "/settings", product: "KiteSecurity Command Center", preview: true, modules }));
