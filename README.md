...
- App: https://kite-security-command-center.vercel.app
- API: https://kite-security-command-center.vercel.app/api/health
- Live chain read: https://kite-security-command-center.vercel.app/api/chain/stats
...
- Worker runtime (`@kite-security-command-center/worker`) wired into the live API at `POST /api/runs/simulate`.
...
| GET | `/scan` | List items. |
| POST | `/scan` | Create an item (`name`, `description`, `owner` required; `owner` must be a valid EVM address). |
| GET | `/scan/:id` | Fetch one item. |
